...
La actualización de la [versión de AEM](/help/implementing/deploying/aem-version-updates.md) siempre es un evento de implementación independiente de la inserción de [código personalizado](#customer-releases). Visto de otra manera, las versiones de código personalizado deben probarse con la versión de AEM que está en producción, ya que es la que se implementa en primer lugar. Las actualizaciones de la versión de AEM que se produzcan después, que son frecuentes, se aplican automáticamente. Están pensadas para ser compatibles con el código de cliente ya implementado.
...
Se aplican algunas restricciones adicionales a estos paquetes de código, por ejemplo, [instalar ganchos](https://jackrabbit.apache.org/filevault/installhooks.html) no son compatibles.
...
>additional-url="https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/migration-journey/cloud-migration/content-transfer-tool/overview-content-transfer-tool.html" text="Herramienta de transferencia de contenido"
...
* Los modos de ejecución de la configuración OSGI deben hacer referencia al RDE, al desarrollo, a la fase, la producción para el entorno o la creación, publicación del servicio. Una combinación de `<service>.<environment_type>` es compatible, mientras que estos entornos deben utilizarse en este orden concreto (por ejemplo, `author.dev` o `publish.prod`). Se debe hacer referencia a los tokens OSGI directamente desde el código en lugar de usar el método `getRunModes` que ya no incluirá el `environment_type` en el tiempo de ejecución. Para obtener más información, consulte [Configuración de OSGi para AEM as a Cloud Service](/help/implementing/deploying/configuring-osgi.md).
...
Las configuraciones de la tarea de mantenimiento deben mantenerse en el control de origen, ya que la pantalla **Herramientas > Operaciones** no estará disponible en los entornos de Cloud. Esta ventaja garantiza la persistencia intencionada de los cambios en lugar de su aplicación reactiva y olvido. Consulte el [artículo Tarea de mantenimiento](/help/operations/maintenance.md) para obtener más información.
