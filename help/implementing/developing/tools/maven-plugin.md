...
>[!TIP]
>
>Para obtener más información, consulte el artículo [AEM Estructura](https://docs.adobe.com/content/help/es-ES/experience-manager-cloud-service/implementing/developing/aem-project-content-package-structure.html) del proyecto en el AEM como documentación de Cloud Service, así como la documentación del [AEM arquetipo](https://docs.adobe.com/content/help/es-ES/experience-manager-core-components/using/developing/archetype/overview.html) del proyecto. Ambos son totalmente compatibles con AEM 6.5.
...
A menos que se indique lo contrario, los objetivos y parámetros del complemento utilizan el `vault` prefijo, como en el siguiente ejemplo:
...
Los objetivos que utilizan proxies para AEM utilizan la primera configuración de proxy válida que se encuentra en la configuración de Maven. Si no se encuentra ninguna configuración proxy, no se utiliza ningún proxy. Consulte el `useProxy` parámetro en la sección Parámetros [](#common-parameters) comunes.

### Parámetros comunes {#common-parameters}

Los parámetros de la siguiente tabla son comunes a todos los objetivos excepto cuando se señalan en la columna **Objetivos** .

| Nombre | Tipo | Requerido | Valor predeterminado | Descripción | Objetivos |
|---|---|---|---|---|---|
| `failOnError` | `boolean` | No | `false` | Un valor de `true` hace que la compilación falle cuando se produce un error. Un valor de hace que `false` la compilación ignore el error. | Todos los objetivos excepto `package` |
| `name` | `String` | `build`:: Sí, `install`: No, `rm`: Sí | `build`:: Sin valor predeterminado, `install`: Valor de la `artifactId` propiedad del proyecto Maven | Nombre del paquete en el que se va a actuar | Todos los objetivos excepto `ls` |
...
Todos los parámetros para el objetivo de compilación se describen en la sección Parámetros [](#common-parameters) comunes.

### install {#install}

Instala un paquete en el repositorio. La ejecución de este objetivo no requiere un proyecto Maven. El objetivo está enlazado a la `install` fase del ciclo vital de la compilación de Maven.
...
|---|---|---|---|---|---|
| `artifact` | `String` | No | Valor de la `artifactId` propiedad del proyecto Maven | Una cadena del formulario `groupId:artifactId:version[:packaging]` |
| `artifactId` | `String` | No | Ninguna | ID del artefacto que se va a instalar |
| `groupId` | `String` | No | Ninguna | El `groupId` aspecto del artefacto que se va a instalar |
| `install` | `boolean` | No | `true` | Determina si se desempaqueta automáticamente el paquete al cargarse |
| `localRepository` | `org.apache.maven.artifact.repository.ArtifactRepository` | No | El valor de la variable del `localRepository` sistema | Repositorio local de Maven que no se puede configurar usando la configuración del complemento ya que la propiedad del sistema siempre se utiliza |
...
| `pomRemoteRepositories` | `java.util.List` | Sí | Valor de la `remoteArtifactRepositories` propiedad definida para el proyecto Maven | Este valor no se puede configurar con la configuración del complemento y debe especificarse en el proyecto. |
...
| `failOnMissingEmbed` | `boolean` | Sí | `false` | Un valor de hace que `true` la compilación falle cuando no se encuentra un artefacto incrustado en las dependencias del proyecto. Un valor de hace que `false` la compilación ignore estos errores. |
...
| `filters` | `com.day.jcr.vault.maven.pack.impl.DefaultWorkspaceFilter` | No | Ninguna | Este parámetro contiene elementos de filtro que definen el contenido del paquete. Cuando se ejecutan, los filtros se incluyen en el `filter.xml` archivo. Consulte la sección [Uso de Filtros](#using-filters) más abajo. |
| `finalName` | `java.lang.String` | Sí | La `finalName` definición en el proyecto Maven (fase de compilación) | Nombre del archivo ZIP del paquete generado, sin la extensión del `.zip` archivo |
| `group` | `java.lang.String` | Sí | La `groupID` definición del proyecto Maven | El `groupId` del paquete de contenido generado que forma parte de la ruta de instalación de destinatario para el paquete de contenido |
...
| `properties` | `java.util.Map` | No | Ninguna | Estos parámetros definen propiedades adicionales que se pueden definir en el `properties.xml` archivo. Estas propiedades no pueden sobrescribir las siguientes propiedades predefinidas: `group` (usar `group` parámetro para establecer), `name` (usar `name` parámetro para establecer), `version` (usar `version` parámetro para establecer), `description` (establecer desde la descripción del proyecto), `groupId` (`groupId` del descriptor del proyecto de Maven), `artifactId` (`artifactId` del descriptor del proyecto de Maven), utilizar (usar parámetro para configurar),  (el valor de la propiedad del sistema), `dependencies` `dependencies` `createdBy` `user.name` `created` `requiresRoot` `requiresRoot` `packagePath` (el tiempo actual), usar parámetro para establecer), etc. del grupo y el nombre del paquete) |
| `requiresRoot` | `boolean` | Sí | false | Define si el paquete requiere root. Esto se convertirá en la `requiresRoot` propiedad del `properties.xml` archivo. |
...
Utilice el elemento filtros para definir el contenido del paquete. Los filtros se agregan al `workspaceFilter` elemento del `META-INF/vault/filter.xml` archivo del paquete.
...
Los archivos de origen pueden ubicarse en cualquier lugar del sistema de archivos. En el archivo POM, defina los recursos de compilación para copiar los archivos de origen en el archivo `target/vault-work/META-INF` para incluirlos en el paquete.

El siguiente código POM agrega los archivos de la `META-INF` carpeta del origen del proyecto al paquete:
...
El siguiente código POM sólo agrega una imagen en miniatura al paquete. La imagen en miniatura debe tener un nombre `thumbnail.png`y estar ubicada en la `META-INF/vault/definition` carpeta del paquete. En este ejemplo, el archivo de origen se encuentra en la `/src/main/content/META-INF/vault/definition` carpeta del proyecto:
...
## Uso del arquetipo del proyecto AEM para generar AEM proyectos {#using-archetypes}
