...
* [SPA ](blueprint.md) modelo para los requisitos técnicos de un SPA
* [Introducción a la SPA en AEM con ](getting-started-react.md) Reactancia para un rápido recorrido por una SPA sencilla con React
* [Introducción a SPA en AEM con ](getting-started-angular.md) Angularis para ver rápidamente un SPA sencillo con Angular
...
Para permitir que el autor utilice el editor de páginas para editar el contenido de un SPA, la aplicación SPA debe implementarse para interactuar con el SDK del Editor SPA de AEM. Consulte el documento [Introducción a SPA en AEM uso de React](getting-started-react.md) para obtener información mínima que necesita saber para que el suyo funcione.
...
Se pueden implementar marcos de SPA adicionales para trabajar con el SDK del Editor de AEM SPA. Consulte el documento [Modelo de SPA](blueprint.md) para conocer los requisitos que debe cumplir un módulo para crear una capa específica del marco de trabajo compuesta de módulos, componentes y servicios para trabajar con el Editor de SPA de AEM.

### Uso de varios selectores {#multiple-selectors}

Los selectores personalizados adicionales se pueden definir y utilizar como parte de una SPA desarrollada para el SDK de AEM SPA. Sin embargo, esta compatibilidad requiere que el selector `model` sea el primer selector y la extensión sea `.json` como requiere el exportador JSON.
...
1. Defina un atributo (puede ser cualquiera) en el elemento de envoltorio de contenedor que contiene el texto HTML. En el caso del proyecto WKND SPA, es un elemento `<div>` y el selector que se ha utilizado es `data-rte-editelement`.
