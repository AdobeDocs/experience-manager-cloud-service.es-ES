...
Tenga en cuenta que también debe configurar si la barra de herramientas de ContextHub aparece en modo de Previsualización. Consulte [Mostrar y ocultar la interfaz de usuario de ContextHub](configuring-contexthub.md#showing-and-hiding-the-contexthub-ui).
...
Todos los tipos de tienda son extensiones de la clase [`ContextHub.Store.Core`](contexthub-api.md#contexthub-store-core). Para obtener información sobre cómo crear un nuevo tipo de almacén, consulte [Creación de tiendas personalizadas](extending-contexthub.md#creating-custom-store-candidates). Para obtener información sobre los tipos de almacén de muestra, consulte [Muestra de candidatos de la tienda ContextHub](sample-stores.md).
...
* Primero se utiliza el almacén [geolocalización](sample-stores.md#contexthub-geolocation-sample-store-candidate) para determinar la latitud del usuario.
* A continuación, el elemento de datos del mes del [almacén de información de surferinfo](sample-stores.md#contexthub-surferinfo-sample-store-candidate) determina la estación en la que está en esa latitud.
...
ContextHub proporciona una [página de diagnóstico](contexthub-diagnostics.md) donde puede ver una visión general del marco de ContextHub.
