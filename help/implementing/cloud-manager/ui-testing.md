...
AEM ofrece un conjunto integrado de [Puertas de calidad de Cloud Manager](/help/implementing/cloud-manager/custom-code-quality-rules.md) para garantizar actualizaciones sin problemas en las aplicaciones personalizadas. AEM En particular, las puertas de prueba de TI ya admiten la creación y automatización de pruebas personalizadas mediante API de.

Las pruebas de interfaz de usuario son pruebas basadas en Selenium empaquetadas en una imagen Docker para permitir una amplia variedad de lenguajes y marcos de trabajo (como Java y Maven, Node y WebDriver.io, o cualquier otro marco de trabajo y tecnología creados en Selenium). Además, se puede generar fácilmente un proyecto de pruebas de interfaz de usuario utilizando [AEM El arquetipo del proyecto de la.](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/developing/archetype/overview.html?lang=es)
...
>
>El Adobe también proporciona un ejemplo de módulo de prueba de interfaz de usuario basado en Java y WebDriver. Consulte la [AEM Repositorio de ejemplos de prueba](https://github.com/adobe/aem-test-samples/tree/aem-cloud/ui-selenium-webdriver) para obtener más información.

## Introducción a las pruebas de IU {#get-started-ui-tests}

En esta sección se describen los pasos necesarios para configurar las pruebas de interfaz de usuario para la ejecución en Cloud Manager.

1. Decida el lenguaje de programación que desea utilizar.

   * Para JavaScript y WDIO, utilice el código de ejemplo que se genera automáticamente en la variable `ui.tests` de su repositorio de Cloud Manager.
   ...
   * Para Java y WebDriver, utilice el código de ejemplo del [AEM Repositorio de ejemplos de pruebas.](https://github.com/adobe/aem-test-samples/tree/aem-cloud/ui-selenium-webdriver)

   * Para otros lenguajes de programación, consulte la sección [Generar pruebas de IU](#building-ui-tests) en este documento para configurar el proyecto de prueba.

1. Asegúrese de que las pruebas de IU estén activadas según la sección [Inclusión del cliente](#customer-opt-in) en este documento.
...
Un proyecto de Maven genera un contexto de generación de Docker. Este contexto de generación de Docker describe cómo crear una imagen de Docker que contenga las pruebas de interfaz de usuario, que Cloud Manager utiliza para generar una imagen de Docker que contenga las pruebas de interfaz de usuario reales.
...
Para que Cloud Manager pueda generar y ejecutar sus pruebas de interfaz de usuario, debe incluirse en esta función al agregar un archivo al repositorio.

* El nombre del archivo debe ser `testing.properties`.
* El contenido del archivo debe ser `ui-tests.version=1`.
* El archivo debe estar en el submódulo de Maven para pruebas de interfaz de usuario junto al `pom.xml` del submódulo de pruebas de interfaz de usuario.
...
Si está utilizando los ejemplos proporcionados por el Adobe:

* Para el basado en JavaScript `ui.tests` carpeta generada a partir de [AEM Tipo de archivo del proyecto](https://github.com/adobe/aem-project-archetype/tree/master/src/main/archetype/ui.tests), puede ejecutar el siguiente comando para añadir la configuración necesaria.
...
* Los ejemplos de prueba de Java proporcionados ya tienen establecido el indicador de inclusión.
...
| `AEM_AUTHOR_USERNAME` | `admin` | AEM El nombre de usuario para iniciar sesión en la instancia de autor de la |
| `AEM_AUTHOR_PASSWORD` | `admin` | AEM La contraseña para iniciar sesión en la instancia de autor de la |
| `AEM_PUBLISH_URL` | `http://my-ip:4503/context-path` | La URL de la instancia de publicación de AEM |
| `AEM_PUBLISH_USERNAME` | `admin` | AEM El nombre de usuario para iniciar sesión en la instancia de publicación de la |
| `AEM_PUBLISH_PASSWORD` | `admin` | AEM Contraseña para iniciar sesión en la instancia de publicación de la |
...
Los ejemplos de prueba de Adobe proporcionan funciones de ayuda para acceder a los parámetros de configuración:
...
* Java: Consulte la [Configuración](https://github.com/adobe/aem-test-samples/blob/aem-cloud/ui-selenium-webdriver/test-module/src/main/java/com/adobe/cq/cloud/testing/ui/java/ui/tests/lib/Config.java) clase
...
Los ejemplos de prueba de la IU de Adobe lo gestionan con el script `wait-for-grid.sh`, que se ejecuta al iniciar Docker e inicia la ejecución de prueba real solo una vez que la cuadrícula esté lista.
...
Si se crea un archivo de resultados de prueba durante la ejecución de una prueba de interfaz de usuario, puede descargarlo desde Cloud Manager mediante la variable `Download Details` botón debajo de [**Pruebas de IU personalizadas** paso.](/help/implementing/cloud-manager/deploy-code.md)
...
1. Cargue el archivo en la dirección URL especificada por la `UPLOAD_URL` variable de entorno.
   ...
   * Los ejemplos de prueba de Adobe proporcionan funciones de ayuda para la carga de archivos:
...
## Ejecución local de pruebas de IU {#run-ui-tests-locally}

Antes de activar pruebas de interfaz de usuario en una canalización de Cloud Manager, se recomienda ejecutar las pruebas de interfaz de usuario localmente en [AEM SDK as a Cloud Service de](/help/implementing/developing/introduction/aem-as-a-cloud-service-sdk.md) AEM o en una instancia as a Cloud Service real de la.

### Requisitos previos {#prerequisites}

Las pruebas en Cloud Manager se ejecutarán con un usuario administrador técnico.

Para ejecutar las pruebas de interfaz de usuario desde el equipo local, cree un usuario con permisos de tipo administrador para lograr el mismo comportamiento.

### Ejemplo JavaScript Test {#javascript-sample}

1. Abra un elemento shell y vaya al `ui.tests` carpeta en su repositorio
...
>* Los archivos de registro se almacenan en `target/reports` de su repositorio
>* Debe asegurarse de que está en la última versión de Chrome, ya que la prueba descarga la última versión de Chrome Driver automáticamente para realizar pruebas.
>
>Para obtener más información, consulte la [AEM Repositorio de tipo de archivo del proyecto.](https://github.com/adobe/aem-project-archetype/blob/develop/src/main/archetype/ui.tests/README.md)

### Muestra de prueba de Java {#java-sample}

1. Abra un elemento shell y vaya al `ui.tests/test-module` carpeta en su repositorio
...
>[!NOTE]
>
>* Los archivos de registro se almacenarán en el `target/reports` de su repositorio.
>
>Para obtener más información, consulte la [AEM Repositorio de ejemplos de pruebas.](https://github.com/adobe/aem-test-samples/blob/aem-cloud/ui-selenium-webdriver/README.md)
