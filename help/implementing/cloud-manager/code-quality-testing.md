...
Para obtener más información acerca de los distintos tipos de canalizaciones, consulte [Configuración de la canalización de CI-CD](/help/implementing/cloud-manager/configuring-pipelines/introduction-ci-cd-pipelines.md).
...
>[!NOTE]
>
>Consulte [Definiciones de métricas de SonarQube](https://docs.sonarsource.com/sonarqube-server/user-guide/code-metrics/metrics-definition) para obtener definiciones más detalladas.
...
El proceso de análisis de calidad no está exento de errores y a veces identifica problemas que no son defectos reales. Este estado se denomina **falso positivo**.

En estos casos, el código fuente se puede anotar con la anotación de Java estándar `@SuppressWarnings` que especifica el ID de regla como atributo de anotación. Por ejemplo, un falso positivo común es que la regla SonarQube para detectar contraseñas codificadas puede ser demasiado restrictiva sobre cómo se identifica una contraseña codificada.
