...
Los métodos `Thread.stop()` y `Thread.interrupt()` puede producir problemas difíciles de reproducir y, a veces, vulnerabilidades de seguridad. Su utilización debe ser objeto de un estricto seguimiento y validación. En general, transmitir mensajes es una manera más segura de lograr objetivos similares.
...
Al ejecutar solicitudes HTTP desde una aplicación de Experience Manager, es fundamental asegurarse de que se han configurado los tiempos de espera adecuados para evitar el consumo innecesario de subprocesos. Desafortunadamente, el comportamiento predeterminado del cliente HTTP predeterminado de Java™ (`java.net.HttpUrlConnection`) y el cliente de componentes HTTP Apache que se utiliza con frecuencia no superará el tiempo de espera, por lo que se deben establecer explícitamente los tiempos de espera. Además, como práctica recomendada, estos tiempos de espera no deben superar los 60 segundos.
...
Una idea errónea relativamente común es que `ResourceResolver` Los objetos creados con una sesión de JCR existente no deben cerrarse explícitamente o que, al hacerlo, se cierre la sesión de JCR subyacente. No suele ser el caso. Independientemente de cómo se abra `ResourceResolver`, debe cerrarse cuando ya no se utiliza. Dado que `ResourceResolver` implementa la interfaz `Closeable`, también es posible utilizar la sintaxis `try-with-resources` en lugar de invocar explícitamente `close()`.
...
### Las excepciones capturadas deben registrarse o rechazarse, no ambas {#caught-exceptions-should-be-logged-or-thrown-but-not-both}
...
### Evite las instrucciones de registro seguidas inmediatamente de una sentencia de rechazo {#avoid-having-a-log-statement-immediately-followed-by-a-throw-statement}
...
Como su nombre sugiere, las excepciones de Java™ siempre deben usarse en circunstancias excepcionales. Como resultado, cuando se captura una excepción, es importante asegurarse de que los mensajes de registro se registren en el nivel adecuado, WARN o ERROR. Esto garantiza que esos mensajes aparezcan correctamente en los registros.
...
### No generar la salida a la salida estándar o a un error estándar {#do-not-output-to-standard-output-or-standard-error}
...
El Experience Manager de inicio de sesión siempre se debe realizar mediante el marco de registro (SLF4J). La salida directa a los flujos de error estándar o de salida estándar pierde la información estructural y contextual proporcionada por el marco de registro. A veces, puede causar problemas de rendimiento.
...
### No utilice el programador de Sling {#sonarqube-sling-scheduler}
...
No utilice el Programador de Sling para tareas que requieren una ejecución garantizada. Los trabajos programados de Sling garantizan la ejecución y son más adecuados para los entornos agrupados y no agrupados.
...
### No utilice API obsoletas de Experience Manager {#sonarqube-aem-deprecated}
...
La superficie de la API del Experience Manager se está revisando constantemente para identificar las API para las que se desaconseja el uso y que, por lo tanto, se consideran obsoletas.
...
Sin embargo, hay casos en los que una API está en desuso en el contexto de Experience Manager, pero puede que no esté en desuso en otros contextos. Esta regla identifica esta segunda clase.
...
>[!NOTE]
>
>OakPAL es un marco de trabajo, que valida paquetes de contenido usando un repositorio Oak independiente. Fue desarrollado por un Socio Experience Manager y ganador del premio Experience Manager Rockstar North America de 2019.
...
La API de Experience Manager contiene interfaces y clases de Java™ que solo están pensadas para utilizarse, pero no para implementarse, mediante código personalizado. Por ejemplo, la interfaz de `com.day.cq.wcm.api.Page` solo debe implementarlo el Experience Manager.
...
Varios índices Oak de Experience Manager listos para usar incluyen una configuración Tika y las personalizaciones de estos índices deben incluir una configuración Tika. Esta regla comprueba las personalizaciones de los índices `damAssetLucene`, `lucene`y `graphqlConfig` y genera un problema si el nodo `tika`  falta o si el nodo `tika` no tiene un nodo llamado `config.xml`.
...
Una práctica recomendada de larga data es que la variable `/libs` el árbol de contenido en el repositorio de contenido del Experience Manager debe ser considerado de solo lectura por los clientes. Modificar nodos y propiedades en `/libs` crea un riesgo significativo para las actualizaciones principales y secundarias. Modificaciones a `/libs` debe hacerse por Adobe a través de canales oficiales.
...
Un problema común que ocurre en proyectos complejos es que el mismo componente OSGi se configura varias veces. Este problema crea una ambigüedad sobre qué configuración es aplicable. Esta regla tiene en cuenta el modo de ejecución, ya que solo identifica los problemas en los que el mismo componente se configura varias veces en el mismo modo de ejecución o combinación de modos de ejecución.

>[!NOTE]
>
>Esta regla genera problemas en los que la misma configuración, en la misma ruta, se define en varios paquetes, incluidos casos en los que el mismo paquete se duplica en la lista general de paquetes creados.
>
>Por ejemplo, si la compilación produce paquetes llamados `com.myco:com.myco.ui.apps` y `com.myco:com.myco.all` donde `com.myco:com.myco.all` incrustaciones `com.myco:com.myco.ui.apps`, luego todas las configuraciones dentro de `com.myco:com.myco.ui.apps` se notifican como duplicados.
>
...
La configuración OSGi `com.day.cq.wcm.core.impl.AuthoringUIModeServiceImpl` define el modo de creación predeterminado en Experience Manager. Porque [la IU clásica ha quedado obsoleta desde Experience Manager 6.4](https://experienceleague.adobe.com/docs/experience-manager-64/release-notes/deprecated-removed-features.html?lang=es), ahora se plantea un problema cuando el modo de creación predeterminado se configura en la IU clásica.

### Los componentes con cuadros de diálogo deben tener cuadros de diálogo de IU táctiles {#oakpal-components-dialogs}
...
Los componentes de Experience Manager que tengan un cuadro de diálogo de IU clásica siempre deben tener un cuadro de diálogo correspondiente de IU táctil. Ambas ofrecen una experiencia de creación óptima y son compatibles con el modelo de implementación de Cloud Service, en el que la IU clásica no es compatible. Esta regla verifica los siguientes escenarios:
...
La documentación de Herramientas de modernización de Experience Manager proporciona documentación y herramientas para convertir componentes de la IU clásica a la IU táctil. Consulte [la documentación de las herramientas de modernización de Experience Manager](https://opensource.adobe.com/aem-modernize-tools/) para obtener más información.
...
### No utilice agentes de replicación inversa {#oakpal-reverse-replication}
...
El soporte para la replicación inversa no está disponible en las implementaciones de Cloud Service, tal como se describe como parte de la [notas de la versión.](/help/release-notes/aem-cloud-changes.md#replication-agents)
...
### Los recursos contenidos en bibliotecas de cliente habilitadas para proxy deben estar en una carpeta denominada resources {#oakpal-resources-proxy}
...
Con el paso a los microservicios de recursos para el procesamiento de recursos en el as a Cloud Service de Experience Manager, varios procesos de flujo de trabajo que se utilizaban en las versiones on-premise y AMS de Experience Manager se han vuelto incompatibles o innecesarios.

La herramienta de migración de [Repositorio de GitHub de Recursos as a Cloud Service de Experience Manager](https://github.com/adobe/aem-cloud-migration) se puede utilizar para actualizar los modelos de flujo de trabajo durante la migración a Experience Manager as a Cloud Service.
...
Aunque el uso de las plantillas estáticas es históricamente común en los proyectos de Experience Manager, Adobe recomienda las plantillas editables, ya que proporcionan la mayor flexibilidad y admiten funciones adicionales que no están presentes en las plantillas estáticas. Encontrará más información en el documento [Plantillas de páginas.](/help/implementing/developing/components/templates.md)

La migración de plantillas estáticas a editables se puede automatizar en gran medida mediante el uso de [Herramientas de modernización del Experience Manager.](https://opensource.adobe.com/aem-modernize-tools/)
...
Experience Manager as a Cloud Service aplica una política de nomenclatura estricta para los nombres de modo de ejecución y un orden estricto para dichos modos de ejecución. La lista de modos de ejecución admitidos se encuentra en el documento [Implementación en Experience Manager as a Cloud Service](/help/implementing/deploying/overview.md#runmodes) y cualquier desviación de esto se identifica como un problema.

### Los nodos de definición del índice de búsqueda personalizado deben ser secundarios directos de /oak:index {#oakpal-custom-search}
...
El Experience Manager as a Cloud Service requiere definiciones de índice de búsqueda personalizadas (es decir, nodos del tipo `oak:QueryIndexDefinition`) ser nodos secundarios directos de `/oak:index`. Los índices de otras ubicaciones deben moverse para que sean compatibles con el Experience Manager as a Cloud Service. Encontrará más información sobre los índices de búsqueda en el documento [Búsqueda de contenido e indexación.](/help/operations/indexing.md)

### Los nodos de definición de índice de búsqueda personalizados deben tener una compatVersion de 2 {#oakpal-custom-search-compatVersion}
...
El Experience Manager as a Cloud Service requiere que haya definiciones de índice de búsqueda personalizadas (como nodos de tipo `oak:QueryIndexDefinition`) debe tener la variable `compatVersion` propiedad establecida en `2`. El Experience Manager as a Cloud Service no admite ningún otro valor. Encontrará más información sobre los índices de búsqueda en [Búsqueda de contenido e indexación.](/help/operations/indexing.md)

### Los nodos descendientes de los nodos de definición del índice de búsqueda personalizada deben ser del tipo nt:unstructured {#oakpal-descendent-nodes}
...
Es difícil solucionar problemas cuando un nodo de definición de índice de búsqueda personalizado tiene nodos secundarios sin ordenar. Para evitar esta situación, se recomienda que todos los nodos descendientes de un `oak:QueryIndexDefinition` node be of type `nt:unstructured`.
...
### Los nodos de definición del índice de búsqueda personalizado deben seguir las convenciones de nomenclatura {#oakpal-custom-search-definitions}
...
El Experience Manager as a Cloud Service requiere definiciones de índice de búsqueda personalizadas (es decir, nodos del tipo `oak:QueryIndexDefinition`) debe tener un nombre que siga un patrón específico descrito en el documento [Búsqueda de contenido e indexación.](/help/operations/indexing.md)

### Los nodos de definición de índice de búsqueda personalizados deben utilizar el tipo de índice Lucene  {#oakpal-index-type-lucene}
...
El Experience Manager as a Cloud Service requiere definiciones de índice de búsqueda personalizadas (es decir, nodos del tipo `oak:QueryIndexDefinition`) tienen un `type` propiedad con el valor establecido en `lucene`. La indexación con tipos de índice heredados debe actualizarse antes de la migración a Experience Manager as a Cloud Service. Consulte [Búsqueda de contenido e indexación](/help/operations/indexing.md#how-to-use) para obtener más información.
...
El Experience Manager as a Cloud Service prohíbe las definiciones de índice de búsqueda personalizadas (es decir, los nodos del tipo `oak:QueryIndexDefinition`) desde que contiene una propiedad denominada `seed`. La indexación con esta propiedad debe actualizarse antes de la migración a Experience Manager as a Cloud Service. Consulte el documento [Búsqueda de contenido e indexación](/help/operations/indexing.md#how-to-use) para obtener más información.
...
El Experience Manager as a Cloud Service prohíbe las definiciones de índice de búsqueda personalizadas (es decir, los nodos del tipo `oak:QueryIndexDefinition`) desde que contiene una propiedad denominada `reindex`. La indexación con esta propiedad debe actualizarse antes de la migración a Experience Manager as a Cloud Service. Consulte el documento [Búsqueda de contenido e indexación](/help/operations/indexing.md#how-to-use) para obtener más información.
