...
Conozca los pasos que permiten la experiencia as a Cloud Service completa de AEM Screens en su sitio de demostración.
...
AEM En el documento anterior del recorrido del complemento Demostraciones de referencia de la, [Creación de un sitio de demostración,](create-site.md) ha creado un sitio de demostración basado en las plantillas del complemento Demostraciones de referencia. Ahora debería hacer lo siguiente:
...
Ahora que tiene su propio sitio de demostración para explorar y comprender las herramientas que le ayudarán a administrar este tipo de sitios, habilite la experiencia as a Cloud Service completa de AEM Screens para ellos.

## Objetivo {#objective}

AEM El complemento Demostraciones de referencia de la incluye contenido de AEM Screens para We.Cafe, un sector comercial de cafeterías. Este documento le ayuda a comprender cómo ejecutar la configuración de demostración de We.Cafe en el contexto de AEM Screens. Después de leer, debería:
...
AEM Al instalar el complemento Demostraciones de referencia de la plataforma de pruebas, tendrá automáticamente el contenido de We.Cafe para AEM Screens disponible en el entorno de creación de demostración. Los pasos descritos en la sección [Implementación de un proyecto de Screens de demostración](#deploy-project) le ayuda a activar la experiencia completa de AEM Screens publicando ese contenido e implementándolo en reproductores de medios, etc.
...
>[!NOTE]
>
>Si decide conectarse al Cloud Service de Pantallas en la [Conectar pantallas as a Cloud Service](#connect-screens) , cree las ubicaciones como carpetas en pantallas. Consulte la sección [Recursos adicionales](#additional-resources) al final de este documento para obtener más información sobre las visualizaciones.
...
La visualización de la entrada está dividida por el día y solo cambia la primera imagen de la mañana a la tarde. En cada paso de la secuencia, también anuncia una preparación especial de café diferente, utilizando una secuencia incrustada medida para reproducir un elemento diferente cada vez.

La última imagen de los canales de entrada también está dirigida (es decir, modificada dinámicamente) en función de la temperatura exterior, que se puede simular como se describe en la [Crear fuente de datos simulada](#data-source) sección.
...
Una vez finalizado el asistente, encontrará el contenido implementado en Sitios y puede navegar y explorar como lo haría con cualquier otro contenido.
...
Debido a que es difícil cambiar el clima durante una demostración o durante las pruebas, los cambios de temperatura deben simularse. Un servicio meteorológico se simula almacenando un valor de temperatura en una hoja de cálculo de hojas de cálculo de Google AEM que ContextHub llama para recuperar la temperatura.
...
1. Para crear un proyecto, haga clic en el nombre del proyecto actual en la parte superior izquierda de la barra de herramientas, después de la etiqueta **Google Cloud Platform** etiqueta.
...
1. Asegúrese de que el nuevo proyecto esté seleccionado y, a continuación, utilizando el menú de hamburguesa del panel de la consola de Cloud, seleccione **API y servicios**.
...
1. En el cuadro de diálogo, copie la nueva clave de API y guárdela para usarla más adelante. Clic **CERRAR** para poder salir del cuadro de diálogo.
...
1. En la pantalla Biblioteca de API, desplácese hasta encontrar la búsqueda **API de hojas de Google** y, a continuación, haga clic en él.

   ![Búsqueda en bibliotecas de API](assets/api-library-search.png)

1. En el **API de hojas de Google** , haga clic en **HABILITAR**.
...
1. Ir a `https://docs.google.com` y cree una hoja de cálculo de Google Sheets.
1. Defina la temperatura introduciendo `32` en la celda A2.
1. Comparta el documento haciendo clic en **Compartir** en la parte superior derecha de la ventana y debajo de **Obtener vínculo**, haga clic en **Cambiar**.
...
AEM Screens puede cambiar el contenido dinámicamente según el contexto. AEM La demostración de We.Cafe tiene canales configurados para mostrar contenido diferente según la temperatura actual mediante el uso de ContextHub de la.
...
Cuando se muestra el contenido de la pantalla, ContextHub llama al servicio meteorológico para buscar la temperatura actual y determinar qué contenido mostrar.
...
1. La configuración ya debería tener los datos JSON preconfigurados. Hay dos valores que deben cambiarse:
   1. Reemplazar `[your Google Sheets id]` con el ID de hoja que [que guardó anteriormente](#create-spreadsheet).
   1. Reemplazar `[your Google API Key]` con la clave de API que [que guardó anteriormente](#create-api-key).
1. Haga clic en **Guardar**.

Ahora puede cambiar el valor de temperatura en la hoja de cálculo de Hojas de cálculo de Google y ContextHub actualiza Screens dinámicamente a medida que &quot;observa cambios metereológicos&quot;.
...
1. Clic **Editar** en la barra de herramientas o escriba la tecla de método abreviado `e` para que pueda editar la página.

1. En el editor, puede ver el contenido. Una imagen se resalta en azul con un icono de direccionamiento en la esquina.
...
En primer lugar, debe publicar el contenido de la demostración de Screens en AEM Screens as a Cloud Service y configurar el servicio.
...
1. Cerca de la esquina superior izquierda, haga clic en **Editar configuración** icono, en forma de engranaje.
...
1. Una vez conectado a las instancias de demostración, Screens extrae el contenido del canal. Clic **Canales** en el panel izquierdo para poder ver los canales publicados. La información puede tardar un momento en rellenarse. Puede hacer clic en el botón azul **Sincronizar** en la parte superior derecha de la pantalla para actualizar la información.

   ![Información del canal de demostración](assets/screens-channels.png)

1. Clic **Visualizaciones** en el panel izquierdo. Todavía no ha creado ninguna para la demostración. Puede simular las ubicaciones de We.Cafe creando carpetas para cada una. Clic **Crear** en la parte superior derecha de la pantalla y seleccione **Carpeta**.

   ![Crear pantalla](assets/screens-displays.png)

1. En el cuadro de diálogo, proporcione un nombre de carpeta como **San Jose** y haga clic en **Crear**.

1. Abra la carpeta haciendo clic en ella y, a continuación, en **Crear** en la parte superior derecha y seleccione **Mostrar**.

1. Proporcione un nombre y haga clic en **Crear**.

   ![Crear pantalla](assets/create-display.png)

1. Una vez creada la pantalla, haga clic en el nombre de la pantalla para abrir la pantalla de detalles de visualización. Se debe asignar a la pantalla un canal que se haya sincronizado desde el sitio de demostración. Clic **Asignar canal** en la parte superior derecha de la pantalla.
...
Para ver el contenido como en una pantalla real, puede descargar el reproductor y configurarlo localmente. AEM Screens as a Cloud Service ofrece el contenido a su reproductor

#### Generar un código de registro {#registration-code}

En primer lugar, debe crear un código de registro para conectar de forma segura un reproductor a AEM Screens as a Cloud Service.
...
1. Ejecute el reproductor y, a continuación, cambie a. **Configuración** pestaña.
1. Desplácese hasta la parte inferior y, a continuación, haga clic en y confirme ambos **Restablecer a fábrica** y **Cambiar a modo de nube** opciones.

   ![Configuración del reproductor](assets/player-configuration.png)

1. El reproductor cambia automáticamente al **Registro del reproductor** pestaña. Introduzca el código que ha generado anteriormente y haga clic en **Registrar**.
...
1. Haga clic en el nombre del reproductor para poder abrir sus detalles. Clic **Asignar a pantalla** en la parte superior derecha de la pantalla.

   ![Asignar reproductor a la pantalla](assets/assign-to-display.png)

1. En el cuadro de diálogo, seleccione la pantalla que creó anteriormente y haga clic en **Seleccionar**.
