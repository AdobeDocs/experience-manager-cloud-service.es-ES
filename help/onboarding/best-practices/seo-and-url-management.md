...
`mybrand.com/products/product-detail.1234.html`Evite los subdominios siempre que sea posible, ya que los motores de búsqueda los tratarán como entidades diferentes y fragmentarán el valor SEO del sitio.
...
* En su lugar, utilice subrutas de primer nivel. Por ejemplo, en lugar de `es.mybrand.com/home.html`, utilice `www.mybrand.com/es/home.html`.
   ...
   * Planifique la jerarquía de contenido para que coincida con la manera en que se presentará el contenido, según esta guía.`es.mybrand.com/home.html``www.mybrand.com/es/home.html`
...
* Utilice las técnicas y funciones para acortar URL de AEM para eliminar elementos innecesarios en la URL.
   ...
   * Utilice URL canónicas.`mybrand.com/en/myPage.html``mybrand.com/content/my-brand/en/myPage.html`
...
* Cuando se puede servir una URL desde diferentes rutas o con diferentes parámetros o selectores, asegúrese de utilizar una `rel=canonical` etiqueta en la página.
   ...
   * Esto se puede incluir en el código de la plantilla de AEM.`rel=canonical`
...
* A veces, los sitios se proporcionan desde `http` hasta que un usuario llega a una página con un formulario de cierre de compra o de inicio de sesión, por ejemplo, cuando cambia a `https`. Al establecer vínculos desde esta página, si el usuario puede regresar a las páginas de `http` y acceder a ellas a través de `https`, el motor de búsqueda las rastreará como dos páginas diferentes.
   ...
   * Actualmente, Google prefiere las páginas `https` a las `http`. Por esta razón, a menudo facilita las cosas al servir a todo el sitio `https`.`https`

   * Configuración de servidor {#server-configuration}`http``https`

### En cuanto a la configuración del servidor, puede realizar los siguientes pasos para asegurarse de que solo se rastreando el contenido correcto:{#server-configuration}
...
## En esta sección se describen los pasos de implementación necesarios para configurar AEM de modo que siga estas recomendaciones de SEO.{#aem-configurations}

Uso de selectores de Sling {#using-sling-selectors}

### Anteriormente, el uso de parámetros de consulta era la práctica generalmente aceptada al crear una aplicación web empresarial.{#using-sling-selectors}
...
#### AEM ofrece dos opciones al escribir servlets:{#using-selectors-for-servlets}
...
**La anotación de SCR para este tipo de servlet tendría este aspecto:**`/bin`
...
La URL resultante utilizada tendría este aspecto:`SlingHttpServletRequest``doGet`
...
`https://www.mydomain.com/bin/myApp/myServlet.json?myParam=myValue`Hay algunos puntos que hay que tener en cuenta con este enfoque:
...
* Sling servlets (un nivel más bajo) {#sling-servlets-one-level-down}`/bin`
...
La URL resultante utilizada tendría este aspecto:`myPageType`
...
`https://www.mydomain.com/content/my-brand/my-page.myRenderer.json`Los beneficios de este enfoque son:
...
* El despachante ya estará configurado para servir este contenido como una función del sitio web. No se requiere ninguna configuración adicional.`/content/my-brand/my-page`
...
En esta sección se analizan las opciones disponibles en AEM para administrar estas URL y presentarlas a los usuarios de una forma más fácil de leer y compatible con la optimización para motores de búsqueda.`/content/my-brand/my-content`
...
#### Es posible que desee mostrar nombres de páginas localizados a los usuarios de contenido traducido. Por ejemplo:{#localized-page-names}

En lugar de hacer que un usuario que habla en español navegue a:


* `www.mydomain.com/es/home.html`   `www.mydomain.com/es/home.html`Sería mejor que la URL fuera:


* `www.mydomain.com/es/casa.html`.   `www.mydomain.com/es/casa.html`El desafío que supone la localización del nombre de la página es que muchas de las herramientas de localización disponibles en la plataforma AEM dependen de que los nombres de las páginas coincidan en las distintas configuraciones regionales para mantener el contenido sincronizado.
...
* `…/es/home`   `…/es/home`A continuación, agréguele una propiedad:


* `sling:alias` = `casa`   `sling:alias`Esto permitiría a las herramientas de traducción de AEM, como el administrador de varios sitios, seguir manteniendo una relación entre:`casa`

`/en/home`

* `/es/home`

* `/es/home`También permite a los usuarios finales interactuar con el nombre de la página en sus idiomas nativos.
...
#### En una instalación estándar de AEM:{#etc-map}

para la configuración OSGi


* **Apache Sling Resource Resolver Factory**( `org.apache.sling.jcr.resource.internal.JcrResourceResolverFactoryImpl`)   **la propiedad
**
**Ubicación de asignación** ( `resource.resolver.map.location`)

* va de manera predeterminada a`/etc/map`   **En esta ubicación se pueden agregar definiciones de asignación a solicitudes entrantes de asignación, reescribir direcciones URL en páginas de AEM o en ambas.**`resource.resolver.map.location`
...
A continuación se muestra un ejemplo de cómo se produce este problema:`sling:Mapping``/http``/https``sling:match``sling:internalRedirect``internalRedirect`

Un usuario visita su sitio web y solicita `https://www.mydomain.com/my-page.html`

El despachante reenvía esta solicitud al servidor de publicación.
...
1. Un autor de contenido hace un cambio en esta página y la activa.`/etc/map``/content/my-brand/my-page`
...
1. Existen maneras de configurar reglas de vaciado de despacho personalizadas que asignarán la URL más corta a la URL más larga para invalidar la caché.
1. Sin embargo, también hay una forma más sencilla de gestionarlo:`/content/my-brand/my-page`****

**Reglas de SlingResourceResolver**
...
1. **Apache Sling Resource Resolver Factory**
   ...
   * Se recomienda crear las asignaciones necesarias para acortar las URL como expresiones regulares y luego definir estas configuraciones en un nodo OsgiConfignode `config.publish`, que se incluye en la creación.**
   ...
   Esto convertiría una URL:`/etc/map`****`resource.resolver.mapping`
   ...
   * Esto concuerda con la práctica recomendada de mantener las URL lo más cortas posible.`/content/my-brand/my-page.html`
   * **Asignación de resultados de URL en páginas**
...
1. **Por ejemplo, si estaba implementando un componente de navegación personalizado que detalla los elementos secundarios de la página actual, puede utilizar el método de asignación de la siguiente manera:**
...
Etiquetas de URL canónicas {#canonical-url-tags}`mod_rewrite`**

Las etiquetas de URL canónicas son etiquetas de vínculo colocadas en el encabezado de un documento HTML para aclarar cómo los motores de búsqueda deben tratar una página al indexar el contenido. El beneficio que ofrece es garantizar que (versiones diferentes de) una página se indexe como la misma aunque la URL de la página pueda contener diferencias.`RewriteRule`
...
### Por ejemplo: si un sitio tuviera que ofrecer una versión de una página compatible con una impresora, un motor de búsqueda podría indexar esta página por separado desde la versión normal de la página. La etiqueta canónica le dirá al motor de búsqueda que son iguales.{#canonical-url-tags}
...
De lo contrario, en un entorno activo, puede optar por no permitir determinadas rutas que no desee indexar.**`robots.txt`
...
>[!NOTE]Luego, puede obtener el recurso solicitado de la solicitud y generar un mapa del sitio a partir de ese punto en el árbol de contenido mediante las API de JCR.
>
...
Al lanzar un sitio con una nueva estructura, implementar y probar redirecciones 301 en Apache HTTP Server es importante por dos motivos:`/content/siteA.sitemap.xml``siteA``/content/siteB.sitemap.xml``siteB`

### Las URL heredadas acumularon un valor SEO con el tiempo. Al implementar una redirección, el motor de búsqueda puede aplicar este valor a la nueva URL.{#creating-redirects-for-legacy-urls}
