...
* Algunos usuarios pueden tener problemas al crear formularios en `Cloud Service version 2024.5.16461`Sin embargo, el problema es que, cuando un usuario crea un formulario, aparece el siguiente mensaje de error en el cuadro de diálogo de creación:
  ...
  Siga los pasos dados para resolver el problema:
   ...
   1. Asegúrese de que la plantilla utilizada en el formulario no tenga el `jcr:uuid` y otras propiedades protegidas generadas por el sistema en la ruta `<template-path>/initial/jcr:content` nodo.
   ...
   1. Si la plantilla utilizada se actualiza en futuras versiones, vuelva a publicarla (como se indica en el paso 2) para evitar problemas de error en la creación del formulario.
