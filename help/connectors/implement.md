...
AEM A continuación se proporcionan referencias útiles para la creación de conectores de la red de distribución y deben leerse con instrucciones sobre [enviar](submit.md) y [mantener](maintain.md) conectores.

AEM Se puede obtener una licencia de desarrollador para la a través del [Programa de Adobe Exchange](https://partners.adobe.com/technologyprogram/experiencecloud.html).
...
* Exportación de datos de AEM a un sistema externo. AEM Por ejemplo, la configuración de suscripción a la newsletter se envía en un sitio web con tecnología de a un CRM.
...
* AEM Análisis de un sitio, página o recurso en el Admin Console de la. Por ejemplo, hacer recomendaciones de SEO para una página existente o no publicada.
...
El Adobe recomienda colocar la mayor parte del código del conector en `/apps/connectors/<vendor>` para mantener una estructura de repositorio limpia, especialmente para los clientes que utilizan varios conectores.
...
Se deben crear nuevos conectores (o se deben modificar los conectores existentes) mediante técnicas de desarrollo de entorno local. AEM El equipo de socios proporciona a los socios de ISV un entorno de zona protegida en el que pueden implementar su conector de la red de distribución de datos (Connector) en una aplicación convencional para garantizar que funcione.
