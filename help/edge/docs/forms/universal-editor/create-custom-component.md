...
El componente personalizado para el bloque de formularios sigue un patrón de arquitectura **MVC (Model-View-Controller)**:
...
- Cada componente de OOTB tiene un archivo JSON que define sus propiedades de creación (por ejemplo, ` _text-input.json`,`_drop-down.json`).
...
Si desea crear componentes personalizados, puede usar la **CLI de Scaffolder** a fin de configurar los archivos y carpetas necesarios para el componente y, a continuación, añadir código para el componente personalizado.

- Los componentes personalizados se localizan en la carpeta `blocks/form/components`.
...
- Añada solo campos nuevos o personalizados de forma explícita en el contenedor. Esto mantiene su esquema DRY y enfocado.
...
- Haga referencia a contenedores de ayuda y otro contenido compartido (por ejemplo, `../form-common/_help-container.json`) según sea necesario para mantener la coherencia y la capacidad de mantenimiento.
...
> - Compruebe siempre los contenedores compartidos existentes o las definiciones de campo antes de añadir nuevas.
...
- Todas las propiedades nuevas añadidas en la definición JSON están disponibles en la definición del campo como propiedades. `<propertyName>` en la lógica JS de su componente
...
La función **Decorar** es el punto de entrada para el componente personalizado. Inicializa el componente, lo vincula con su definición JSON y le permite manipular su estructura y comportamiento de HTML.
...
- **Modificar el elemento**: añada detectores de eventos, actualice atributos o inserte marcado adicional.
...
La función **suscribir** permite que su componente reaccione a los cambios en los valores de los campos o a los eventos personalizados. Esto garantiza que el componente permanezca sincronizado con el modelo de datos del formulario y pueda actualizar dinámicamente su interfaz de usuario.
...
- **Registrar una llamada de retorno**: al llamar a **suscribir(element, formId, callback)**, se registra su llamada de retorno para que se ejecute siempre que cambien los datos del campo. Use dos parámetros de llamada de retorno:
...
- **Escuchar cambios o eventos**: use `fieldModel.subscribe((event) => { ... }, 'eventName')` para ejecutar la lógica cada vez que cambie un valor o se active un evento personalizado. El objeto de evento contiene datos sobre los cambios.
...
- Añade tarjetas a los filtros para permitir la adición dentro del bloque de formulario adaptable.
...
1. **Actualice el editor universal**: abra el formulario en el editor universal y actualice la página para asegurarse de que carga el código más reciente del repositorio.

2. **Añadir el componente personalizado**

   1. Haga clic en el botón **Añadir (+)** en el lienzo del formulario.
...
2. Abra el servidor proxy iniciado en `http://localhost:3000/{path-to-your-form}` (ejemplo de ruta de acceso: `/content/forms/af/custom-component-form`)
...
Vamos a añadir una clase **card** al componente para aplicar estilo y añadir una imagen para cada radio; use el siguiente código para esto.
...
Ahora, el componente de cards aparece de esta manera:

![add card css and js](/help/edge/docs/forms/universal-editor/assets/add-card-css.png)

#### 2.2 Añadir un comportamiento dinámico mediante la función Suscribirse
...
Cuando se llama a la API, esta establece el modelo de campo y debe escuchar los cambios y procesar la vista en consecuencia. Esto se logra usando la **función de suscripción**.
...
Para sincronizar los cambios de vista con el modelo de campo, debe establecer el valor de la tarjeta seleccionada. De este modo, añada el siguiente detector de eventos de cambio en cards.js, como se muestra a continuación:

**Uso de API de modelo de campo en cards.js**
...
![Componente de tarjeta personalizada](/help/edge/docs/forms/universal-editor/assets/cc-ue-card-component.png)
...
1. **Elija un componente OOTB** para extender (por ejemplo, botón, lista desplegable, entrada de texto, etc.). En este caso, amplíe el componente de opción.

2. **Cree una carpeta** en `blocks/form/components` con el nombre de su componente (en este caso, tarjetas).

3. **Añada un archivo JS** con el mismo nombre:
   - `blocks/form/components/cards/cards.js`.

4. (Opcional) **Añada un archivo CSS** para los estilos personalizados:
...
5. **Defina un nuevo archivo JSON** (por ejemplo,` _cards.json`) en la misma carpeta que el **archivo JS de componente** (`blocks/form/components/cards/_cards.json`). Este JSON debe ampliar un componente existente y, en sus definiciones, establecer `fd:viewType` en el nombre de su componente (en este caso, tarjetas):
   ...
   - Para todos los grupos de campos (básico, de validación, de ayuda, etc.), añada explícitamente los campos personalizados.

6. **Implemente la lógica JS y CSS:**
...
7. Registre el componente como una variante en el generador de formularios y establezca la propiedad de variante o
   `fd:viewType/:type` en el JSON en el nombre de su componente, por ejemplo, añada el valor `fd:viewType` del `definitions[]` como tarjetas a la matriz de componentes del objeto con `id="form`.
...
8. **Actualice mappings.js**: añada el nombre de su componente a **OOTBComponentDecorators** (para componentes de estilo OOTB) o a la lista **customComponents** para que el sistema lo reconozca y lo cargue.
...
9. **Actualice _form.json**: añada el nombre de su componente a la matriz `filters.components` para que se pueda colocar en la interfaz de usuario de creación.
...
10. **Actualice _component-definition.json**: en `models/_component-definition.json`, actualice la matriz dentro del grupo con `id custom-components` con un objeto de la siguiente manera:
...
11. **Ejecute la compilación:json script**: Ejecute `npm run build:json` para compilar y combinar todas las definiciones JSON de componentes en un solo archivo que se servirá desde el servidor. Esto garantiza que el esquema del nuevo componente se incluya en la salida combinada.

12. Confirme y envíe los cambios al repositorio de Git.

Ahora puede añadir el componente personalizado al formulario.
...
- **Mantenga centrada la lógica de su componente**: añada o anule únicamente lo necesario para el comportamiento personalizado

- **Aproveche la estructura base**: use el HTML OOTB como punto de partida

- **Use propiedades autorizables:** exponga opciones configurables a través del esquema JSON
...
- **blocks/form/models/form-components**: definiciones de propiedades de componentes personalizados y OOTB.

- **blocks/form/components**: lugar para los componentes personalizados. Por ejemplo: `blocks/form/components/countdown-timer/_countdown-timer.json` muestra cómo ampliar un componente base y añadir nuevas propiedades.
