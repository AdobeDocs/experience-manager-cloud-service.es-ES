...
1. En la barra de herramientas de la parte superior de la página, toque o haga clic en el menú desplegable de modo para mostrar los tipos de modo disponibles.

   ![Modo Targeting](../assets/targeted-mode.png)

1. Toque o haga clic en **Segmentación**. Las opciones de orientación se muestran en la parte superior de la página.

   ![Barra de herramientas de objetivos](../assets/targeted-toolbar.png)
...
>[!NOTE]
>
>Para utilizar el proceso de segmentación, debe ser miembro del grupo de usuarios Autores de actividades de Target .
...
1. Para mostrar los controles que lo guiarán a través del proceso de orientación, toque o haga clic en **Iniciar orientación**.
   ...
   ![Iniciar segmentación](../assets/targeted-start-targeting.png)
...
La experiencia predeterminada también se muestra cuando las propiedades del visitante no coinciden con ningún segmento asignado a las experiencias. Consulte [Adición de experiencias con el modo de Orientación](#adding-and-removing-experiences-using-targeting-mode).
...
1. De forma predeterminada, no se establece ningún identificador de ubicación. Toque o haga clic en la rueda de engranaje de configuración para definir la ubicación.
   ...
   >[!NOTE]
   >
   >Si el administrador lo configura, es posible que tenga que establecer la ubicación explícitamente.
   >
...
### Si utiliza AEM segmentación o Adobe Target (segmentación de experiencias) {#if-you-are-using-aem-targeting-or-adobe-target-experience-targeting}
...
![Segmentación A/B](../assets/targeted-ab.png)
...
1. Si ha configurado Adobe Analytics con su cuenta de Adobe Target, verá el menú desplegable **Fuente de informes**. Seleccione **Adobe Target** o **Adobe Analytics** como fuente.
   ...
   ![Fuente de informes](../assets/targeted-reporting-source.png)
...
1. Si desea tener más control sobre cómo se cuentan las métricas de éxito, toque o haga clic en **Configuración avanzada** para acceder a ellas.
...
| Opción | Descripción |
|---|---|
| Direccionamiento preciso | Al habilitar la orientación precisa, se indica al componente que espere a que los datos de Client Context o Context Hub estén disponibles antes de enviar la solicitud a Adobe Target. Puede aumentar el tiempo de carga. Para la creación, la orientación precisa siempre está activada. Si selecciona la casilla Orientación precisa , el mbox realiza primero un mboxDefine y luego un mboxUpdate, lo que da como resultado una solicitud de Ajax una vez que los datos están disponibles. Si no selecciona la casilla Orientación precisa , el mbox realiza un mboxCreate, lo que da como resultado una solicitud sincrónica inmediatamente (en este caso, es posible que no todos los datos de contexto estén disponibles aún). Nota: La activación o desactivación de la orientación precisa en un componente específico no afecta a la configuración que haya establecido globalmente. Siempre puede seleccionar Orientación precisa en el componente para anular la configuración global. |
| Incluir segmentos resueltos | Al seleccionar esta casilla de verificación, se incluyen todos los segmentos resueltos en la llamada de mbox y todos los parámetros configurados en la página y en la infraestructura. Esto solo funciona en situaciones con la API XML donde está sincronizando AEM segmentos. Si dispone de segmentos en AEM que Adobe Target no controla (como segmentos de script), esta opción le permite resolver el segmento en AEM y enviar información a Adobe Target de que el segmento está activo. |
...
| Parámetros de contexto | Toque o haga clic en Agregar campo para configurar parámetros de contexto adicionales (lo mismo que está disponible en la infraestructura de Target). Los parámetros de contexto añadidos al componente se aplican solo al componente y no a otros componentes, como sería el caso si hubiera añadido los parámetros de contexto directamente a la infraestructura. |
| Parámetros estáticos | Toque o haga clic en Agregar campo para configurar parámetros estáticos adicionales (lo mismo que está disponible en la infraestructura de Target). Los parámetros estáticos añadidos al componente solo se aplican al componente y no a otros componentes, como sería el caso si hubiera añadido los parámetros estáticos directamente a la infraestructura. Los parámetros estáticos no proceden del contexto (Client Context o ContextHub). |
