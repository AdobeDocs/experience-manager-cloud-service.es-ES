---
title: Administrar públicos
description: La consola Públicos permite crear, organizar y administrar los públicos de la cuenta de Adobe Target o administrar segmentos de ContextHub
...
# Administrar públicos{#managing-audiences}

La consola Públicos permite crear, organizar y administrar los públicos de la cuenta de Adobe Target o administrar segmentos de ContextHub:

* Añadir públicos: de Adobe Target o segmentos de ContextHub.
* Administrar públicos.

Un público, denominado *segmento* en ContextHub, es una clase de visitantes definidos por criterios específicos y que, a su vez, determina quién verá una actividad objetivo. Al segmentar una actividad, puede seleccionar el público directamente en el proceso de segmentación o crear una nueva en la consola de públicos.

En la consola de públicos, estos se organizan por marca.

Los públicos están disponibles en el modo Segmentación del [contenido objetivo de creación](/help/sites-cloud/authoring/personalization/targeted-content.md), donde también puede crear públicos (debe crear los públicos de Adobe Target en la consola Públicos). Los públicos que crea en el modo Segmentación aparecen en la consola de públicos.
...
Para crear un segmento de ContextHub en la consola de públicos:
...
1. En cuadro de diálogo **Nuevo segmento de ContextHub**, introduzca un título, ajuste el aumento y haga clic en **Crear**. El nuevo segmento de ContextHub aparece en la lista de públicos.
...
Al utilizar más de una regla, estas reglas se combinan según el operador boolean AND, lo que significa que cualquier posible miembro del público debe cumplir todas las condiciones que se detallaron para ser incluido en la actividad. Por ejemplo, si define una regla de sistema operativo AND una regla de explorador, solo se incluyen en la actividad los visitantes que usan el sistema operativo definido y el explorador definido.

>[!NOTE]
>
>Si no ve **Crear audiencia de Target** en el menú **Crear**, no tiene los permisos necesarios para crear una audiencia. Debe disponer de permisos de escritura en `/etc/segmentation` para poder crear públicos. Los autores de contenido del grupo tienen permisos de escritura de forma predeterminada.

Para crear un público de Adobe Target:

1. En la consola de navegación, seleccione **Personalization**. Seleccionar **audiencias**.

   ![Navegación a públicos](/help/sites-cloud/authoring/assets/audiences-navigation.png)

1. En la consola de audiencias, seleccione **Crear** y luego **Crear audiencia de destino**.

   ![Creación de un público destinatario](/help/sites-cloud/authoring/assets/audiences-create-target.png)
...
1. Haga clic en **Agregar regla** para añadir otra regla. Escriba tantas reglas como sea necesario. Las reglas se combinan con el operador boolean AND, lo que significa que el público debe cumplir todos los requisitos de cada regla para poder optar a una actividad.
...
Puede editar cualquier público de ContextHub desde la consola Públicos. También puede editar públicos de Adobe Target, pero solo los que se hayan creado en AEM:
