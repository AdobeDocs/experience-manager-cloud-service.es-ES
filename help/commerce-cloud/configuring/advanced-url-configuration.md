...
>[!NOTE]
>
> La optimización de los motores de búsqueda (SEO) se ha convertido en una preocupación clave para muchos expertos en marketing. Como resultado, las preocupaciones de SEO deben abordarse en muchos proyectos as a Cloud Service en Adobe Experience Manager AEM (). Consulte [Prácticas recomendadas para la optimización de los motores de búsqueda y administración URL](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/overview/seo-and-url-management.html) para obtener más información.
...
Este esquema selecciona el `url_path` con la mayoría de los antecesores, basado en la suposición de que una categoría secundaria es más específica que su categoría principal. El seleccionado `url_path` se considera _canónico_ y siempre se utiliza como vínculo canónico en las páginas de productos o en el mapa del sitio del producto.
...
Por ejemplo, considere la respuesta de un [consulta de productos](https://devdocs.magento.com/guides/v2.4/graphql/queries/products.html) más abajo. Dados lo siguiente:
...
Es posible crear lo siguiente [páginas de productos y de varias categorías](../authoring/multi-template-usage.md) solo para un subconjunto específico de categorías o productos de un catálogo.
...
Para ese caso de uso, cada una de las páginas del catálogo puede tener una referencia a una página de producto y categoría específica para la categoría configurada para la página del catálogo. El `UrlProvider` utiliza estas conexiones para crear vínculos para las páginas y categorías de la categoría configurada. Sin embargo, por motivos de rendimiento solo se tienen en cuenta los elementos secundarios de la página de catálogo directa de la raíz de navegación o la página de aterrizaje de un sitio.
...
>[!NOTE]
>
>Esta configuración debe ajustarse con el dominio externo utilizado por el proyecto. Las asignaciones de Sling funcionan según el nombre de host y el dominio. Por lo tanto, esta configuración está deshabilitada de forma predeterminada y debe habilitarse antes de la implementación. Para ello, cambie el nombre de la asignación de Sling `hostname.adobeaemcloud.com` carpeta en `ui.content/src/main/content/jcr_root/etc/map.publish/https` según el nombre de dominio utilizado y habilite esta configuración añadiendo `resource.resolver.map.location="/etc/map.publish"` a la `JcrResourceResolver` configuración del proyecto.
