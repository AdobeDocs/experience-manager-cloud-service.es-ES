...
* Apoyo a la integración con análisis externos y sistemas externos

El visor está disponible como opción adicional y requiere una selección explícita cuando se admite. No reemplaza automáticamente los visores de vídeo existentes.

El nuevo visor de vídeo está destinado a organizaciones que requieren una experiencia de vídeo mejorada y extensible sin interrumpir las implementaciones existentes.
...
|------------|-------------|
| Sincronización dinámica de medios | La carpeta de activos debe sincronizarse con medios dinámicos. |
...
1. Desplácese a **Activos** > **Archivos** y abra la carpeta que contiene el activo de vídeo.
2. Haga clic en el activo de vídeo para abrir la página de detalles del activo.
...
El nuevo visor de vídeo está disponible a través del componente **Dynamic Media** existente en sitios AEM.

### Agregar el componente de medios dinámicos

Ejecute los siguientes pasos para agregar un vídeo utilizando el componente de medios dinámicos:
...
3. En el **cuadro de diálogo de configuración de Dynamic Media**, seleccione **Vídeo (nuevo)** de la lista desplegable **Ajuste preestablecido de visor**.
   ![Seleccionar el visor de vídeo (nuevo) predefinido](assets/viewer-preset.jpeg)

4. Introduzca los modificadores necesarios en el campo **Modificadores del visor** (por ejemplo, `autoplay=true&muted=true`).
   ![Modificadores del visor](assets/additional-modifiers.jpeg)

5. Guarde los cambios.

El vídeo se carga en la página utilizando el Visor de vídeo nuevo.
...
Adobe recomienda validar el origen del mensaje para asegurarse de que los eventos se procesan sólo desde dominios de medios dinámicos de confianza.

## Informe de compromiso de vídeo para el nuevo visor de vídeo {#video-engagement-report}

El Informe de Compromiso de Vídeo proporciona métricas analíticas para los vídeos reproducidos utilizando el nuevo visor de vídeo en medios dinámicos. El informe proporciona datos de rendimiento agregados para el mes especificado y admite la presentación de informes mensuales.

Los informes se generan a petición de los interesados. Para solicitar un informe, cree un [vale de asistencia](https://helpx.adobe.com/es/enterprise/using/support-for-experience-cloud.html) y proporcione los siguientes detalles:
...
|-------|------------|-------------|
| `video_id` | Identificador de vídeo único. | ND |
| `video_name` | Nombre del activo de vídeo. | ND |
...
| `engagement_score` | Porcentaje promedio de observación en todos los eventos de juego. | (Porcentaje total de la cronología de vídeo vista en todas las sesiones/ `video_views`) |
...
La reproducción automática se cuenta como una vista de vídeo. La reproducción iniciada automáticamente se graba como una vista.

+++

+++Si un usuario observa sólo parte de un vídeo (por ejemplo, los primeros 2 segundos y los últimos 2 segundos de un vídeo de 10 segundos), ¿se cuenta como una vista completa?

Una vista se cuenta como completada cuando la reproducción llega al final de la línea de tiempo de vídeo, aunque se hayan omitido partes del vídeo.
