...
1. Para cancelar una carga en curso, haga clic en Cerrar (`X`) junto a la barra de progreso. Al cancelar la operación de carga, [!DNL Assets] elimina la parte del recurso que se ha cargado parcialmente.Si cancela una operación de carga antes de que se carguen los archivos, [!DNL Assets] detiene la carga del archivo actual y actualiza el contenido. Sin embargo, los archivos que ya se han cargado no se eliminan.

1. El cuadro de diálogo de progreso de carga en [!DNL Assets] muestra el recuento de archivos cargados correctamente y los archivos que no se cargaron.Además, la interfaz de usuario [!DNL Assets] muestra el recurso más reciente que ha cargado o la carpeta que creó en primer lugar.
...
* Caracteres no válidos para el nombre de recurso: `* / : [ \\ ] | # % { } ? &` o `;=` (punto y coma seguido de un signo igual)
* Caracteres no válidos para el nombre de la carpeta de recursos: `* / : [ \\ ] | # % { } ? \" . ^ ; + & \t`

Ejemplos de nombres de archivo no válidos:
...
Se requiere una cuenta de almacenamiento externa o un bloque de Azure o AWS para utilizar esta función.
...
1. Proporcione los valores para crear una conexión con el origen de datos. Por ejemplo, si selecciona **Azure Blob Storage** como fuente de datos, especifique los valores de la cuenta de Azure Storage, el contenedor de blob de Azure y la clave de acceso de Azure.

1. Seleccione el modo de autenticación necesario en la lista desplegable. **La clave de acceso de Azure** proporciona acceso completo a la cuenta de almacenamiento de Azure, mientras que el **token SAS de Azure** permite al administrador limitar las capacidades del token mediante permisos y directivas de caducidad.
