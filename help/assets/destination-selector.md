...
El Selector de destino de Micro-Frontend proporciona una interfaz de usuario dentro de su aplicación que se integra fácilmente con el [!DNL Experience Manager Assets as a Cloud Service] repositorio. Puede buscar o examinar la carpeta adecuada en el [!DNL Experience Manager Assets as a Cloud Service] y cargar recursos desde la aplicación.
...
El ámbito de este artículo es demostrar cómo utilizar el Selector de destino con una [!DNL Adobe] aplicación bajo Unified Shell o cuando ya tiene un imsToken generado para la autenticación. Estos flujos de trabajo se denominan flujo no SUSI en este artículo.

Realice las siguientes tareas para integrar y utilizar el Selector de destino con su [!DNL Experience Manager Assets as a Cloud Service] repositorio:
...
Puede definir el Selector de destino [propiedades](#destination-selector-properties) en el `index.html` para personalizar la visualización del Selector de destino en la aplicación.
...
Una vez configurado el Selector de destino y autenticado para utilizar el Selector de destino con su [!DNL Adobe Experience Manager] as a [!DNL Cloud Service] aplicación, puede seleccionar recursos o realizar otras operaciones para buscar los recursos dentro del repositorio.
