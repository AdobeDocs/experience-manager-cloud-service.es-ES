...
* Las etiquetas jerárquicas son extremadamente flexibles y son una excelente manera de organizar los términos de una manera lógica. Mediante espacios de nombres, etiquetas y subetiquetas, se pueden representar sistemas taxonómicos completos.
...
Para poder aplicar etiquetas a los recursos, primero debe crear un espacio de nombres y luego crear y añadirle etiquetas. También puede crear etiquetas y añadirlas a un espacio de nombres existente. Todas las etiquetas que cree en el nivel raíz se añaden automáticamente al espacio de nombres de etiquetas estándar. A continuación, puede añadir el campo Etiquetas al formulario de metadatos para que se muestre en la página Detalles del recurso. Después de configurar estos ajustes, puede comenzar a aplicar etiquetas a los recursos.
...
## Crear un espacio de nombres {#create-a-namespace}

Un espacio de nombres es un contenedor para etiquetas que solo puede existir en el nivel raíz. Puede comenzar a configurar la estructura jerárquica de etiquetas definiendo primero un nombre lógico para el espacio de nombres. Si no añade una etiqueta a ninguno de los espacios de nombres existentes, la etiqueta se mueve a Etiquetas estándar automáticamente.

Siga estos pasos para crear un espacio de nombres:

1. Vaya a `Taxonomy Management` en `Settings` para ver la lista de espacios de nombres existentes. También puede ver la última fecha de modificación, el usuario que modificó el espacio de nombres o las etiquetas debajo de ella y el número de veces que la etiqueta se utiliza en un recurso.
1. Haga clic en `Create Namespace`.
1. Agregue `Title`, `Name` y `Description` al espacio de nombres. La entrada especificada en el campo `Title` se muestra en la parte superior de la jerarquía. Por ejemplo, en la siguiente imagen, **Actividades** hace referencia al título del espacio de nombres.

   ![Administración de etiquetado](assets/tags-hierarchy.png)

1. Haga clic en `Save`.

## Añadir etiquetas a un espacio de nombres {#add-tags-to-namespace}
...
Las etiquetas no estructuradas o las etiquetas que no tienen jerarquía se almacenan en el espacio de nombres `Standard Tags`. Además, si desea añadir términos descriptivos adicionales sin que afecte a la taxonomía controlada, puede almacenar dicho valor en `Standard Tags`. Con el tiempo, puede mover estos valores a espacios de nombres estructurados. Además, puede utilizar el espacio de nombres `Standard Tags` como entrada de forma libre para palabras clave.
...
* Las etiquetas solo se pueden mover por debajo de espacios de nombres existentes o dentro de una jerarquía de etiquetas existente.
* Las etiquetas no se pueden mover a la raíz para convertirlas en un espacio de nombres.
...
1. Seleccione la etiqueta o toda la jerarquía de etiquetas en el espacio de nombres correspondiente y haga clic en `Move`.
1. En el cuadro de diálogo Mover, seleccione la nueva etiqueta de destino o el espacio de nombres mediante la sección `Select Tag`.
...
Puede eliminar varios espacios de nombres o etiquetas simultáneamente. La operación de eliminación no se puede deshacer.

Realice los siguientes pasos para eliminar etiquetas:

1. Seleccione el espacio de nombres o la etiqueta y haga clic en `Delete`.
1. Haga clic en `Confirm`.

>[!NOTE]
>
>* Al eliminar la etiqueta principal o el espacio de nombres, también se eliminan las subetiquetas almacenadas en la jerarquía. Si necesita eliminar o actualizar el espacio de nombres principal, se recomienda [mover las etiquetas](#moving-tags) al nuevo destino antes de eliminar la jerarquía principal.
