...
Puede configurar OAuth para varios proveedores de correo electrónico. AEM A continuación, se proporcionan instrucciones paso a paso para configurar el servicio de correo de para autenticarse mediante OAuth2 con Outlook de Microsoft® Office 365. Otros proveedores se pueden configurar de manera similar.
...
1. Buscar por **Azure Active Directory** en la barra de búsqueda y haga clic en el resultado. También puede navegar directamente a [https://portal.azure.com/#blade/Microsoft_AAD_IAM/ActiveDirectoryMenuBlade/Overview](https://portal.azure.com/#blade/Microsoft_AAD_IAM/ActiveDirectoryMenuBlade/Overview)
1. Clic **Registro de aplicación** > **Nuevo registro**.
...
1. Clic **Añadir permiso** > **Permiso de gráfico** > **Permisos delegados**.
1. Seleccione los siguientes permisos para su aplicación y haga clic en **Añadir permiso**:
   ...
   * `email`
   * `profile`
1. Ir a **Autenticación** > **Añadir una plataforma** > **Web**, y en el **Redirigir direcciones Url** , añada las siguientes direcciones URL: una con barra diagonal y otra sin:
   * `http://localhost/`
   * `http://localhost`
1. Pulse **Configurar** después de añadir cada URL y configurar los ajustes según sus necesidades.
1. A continuación, vaya a **Certificados y secretos**, haga clic en **Nuevo secreto de cliente** y siga los pasos que aparecen en pantalla para crear un secreto. Asegúrese de tomar nota de este secreto para utilizarlo posteriormente.
1. Prensa **Información general** en el panel izquierdo y copie los valores de **ID de aplicación (cliente)** y **ID de directorio (inquilino)** para su uso posterior.

AEM Para recapitular, utilice la siguiente información para configurar OAuth2 para el servicio de correo en el lado del servidor de correo de la parte de la:
...
1. Cuando se le solicite, permita el permiso.
1. La dirección URL redirige a una nueva ubicación, construida con este formato: `http://localhost/?code=<code>&state=12345&session_state=4f984c6b-cc1f-47b9-81b2-66522ea83f81#`.
...
>[!NOTE]
>
> Puede obtener la colección de la API Postman desde [esta ubicación](https://learn.microsoft.com/en-us/azure/active-directory/develop/v2-oauth2-auth-code-flow).
>
> Consulte la documentación de OAuth de MSFT [aquí](https://learn.microsoft.com/es-ES/exchange/client-developer/legacy-protocols/how-to-authenticate-an-imap-pop-smtp-application-by-using-oauth) para obtener más información.
...
1. Rellene el `authUrl`, `tokenUrl`, y `refreshURL` construyéndolos como se describe en la sección anterior.
...
Si el servicio de correo no funciona correctamente, debe volver a generar el `refreshToken` como se ha descrito anteriormente, pasando el nuevo valor a través de la API de Cloud Manager. El nuevo valor tarda unos minutos en implementarse.
